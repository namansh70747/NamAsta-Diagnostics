...
export function ManagePanelsDialog({
  ...
  return (
    <Modal title="Manage Panels" onClose={onClose} width="max-w-xl">
      <p className="text-[13px] text-[#4c4e5d] leading-relaxed mb-4">
      ...
      {panels.length === 0 ? (
        <div className="py-14 text-center">
          ...
          <p className="text-[13.5px] text-[#4c4e5d]">No panels configured.</p>
        </div>
      ) : (
        <div className="rounded-xl border border-[#eef0f4] overflow-hidden">
          <table className="w-full">
            ...
            <tbody>
              {panels.map((p, i) => (
                <tr key={p.id} className="group border-b border-[#e9ebf2] last:border-0 hover:bg-[#fafafe]">
                  <td className="px-4 py-2.5 font-mono text-[12px] text-[#4c4e5d]">{p.code}</td>
                  ...
                  <td className="px-4 py-2.5 text-center">
                    {p.page_break_after ? (
                      ...
                      <span className="text-[12px] text-[#5e6072]">—</span>
                    )}
                  </td>
                  {canManage && (
