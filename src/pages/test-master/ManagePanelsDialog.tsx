...
export function ManagePanelsDialog({
  ...
  return (
    <Modal title="Manage Panels" onClose={onClose} width="max-w-xl">
      <p className="text-[13px] text-[#565869] leading-relaxed mb-4">
      ...
      {panels.length === 0 ? (
        <div className="py-14 text-center">
          ...
          <p className="text-[13.5px] text-[#565869]">No panels configured.</p>
        </div>
      ) : (
            ...
            <tbody>
              {panels.map((p, i) => (
                <tr key={p.id} className="group border-b border-[#f1f1f5] last:border-0 hover:bg-[#fafafe]">
                  <td className="px-4 py-2.5 font-mono text-[12px] text-[#565869]">{p.code}</td>
                  ...
                  <td className="px-4 py-2.5 text-center">
                    {p.page_break_after ? (
                      ...
                      <span className="text-[12px] text-[#6e7081]">—</span>
                    )}
                  </td>
                  {canManage && (
