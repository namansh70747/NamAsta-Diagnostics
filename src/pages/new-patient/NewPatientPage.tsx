        ...
        <div className="card p-6 lg:col-span-3">
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-3">
              ...
              <div className="flex-1">
                <label className={labelCls}>Email <span className="text-[#82849a] font-normal">(optional)</span></label>
          ...
          <div className="card p-6">
                ...
                <div className="card absolute z-10 top-full left-0 right-0 mt-1.5 max-h-60 overflow-y-auto shadow-[var(--shadow-pop)] animate-scale-in py-1">
                  {searchRows.map((row, i) => row.kind === 'group' ? (
                    ...
                    >
                      ...
                      <span className="text-[12px] text-[#6b6c7e] tabular-nums shrink-0">+{row.group.codes.length} tests</span>
                    </button>
                  ) : (
                    <button
                      key={row.test.id}
                    ...
                    >
                      ...
                      <span className="text-[12.5px] text-[#6b6c7e] tabular-nums shrink-0">₹{row.test.price}</span>
                    </button>
                  ))}
                </div>
              )}
            ...
            {frequent.length > 0 && (
              <div className="flex flex-wrap items-center gap-1.5 mb-4">
                <span className="flex items-center gap-1 text-[10.5px] font-semibold uppercase tracking-wide text-[#82849a]">
            ...
            <div className={cn("max-h-56 overflow-y-auto -mx-1 px-1", errors.tests && "mt-2")}>
              {selectedTests.length === 0 ? (
                <div className="py-8 text-center">
                  <p className="text-[13.5px] text-[#6b6c7e]">No tests selected yet</p>
                  <p className="text-[12px] text-[#82849a] mt-1">Search, or tap a panel / frequent chip</p>
                </div>
                ...
                >
                  <span className="min-w-0 truncate">
                    <span className="font-mono text-[11.5px] text-[#82849a] mr-2">{st.test.code}</span>
                    <span className="text-[13.5px] text-[#14151c]">{st.test.name}</span>
                  </span>
                  <span className="flex items-center gap-1 shrink-0">
                    <span className="text-[12.5px] text-[#82849a]">₹</span>
                    <input
                      type="number"
                      tabIndex={-1}
                      value={st.price}
                    ...
                    <button
                      ...
                      className="ml-1 text-[#82849a] opacity-0 group-hover:opacity-100 transition-opacity hover:text-[#b91c1c]"
                      title="Remove test"
                    >
