    ...
    >
      ...
      >
        <div className="flex items-center gap-3 px-4.5 px-5 border-b border-[#eef0f4]">
          <Search size={17} className="text-[#5e6072] shrink-0" />
          <input
            ref={inputRef}
            value={q}
            onChange={e => setQ(e.target.value)}
            placeholder="Search patients by name, receipt no. or phone — or jump to a screen…"
            className="flex-1 py-4 text-[14px] outline-none placeholder:text-[#5e6072] bg-transparent"
            onKeyDown={e => {
          ...
          />
          <kbd className="text-[10px] font-semibold text-[#4c4e5d] bg-[#eef0f4] rounded-md px-1.5 py-1">esc</kbd>
        </div>
        ...
        <div className="max-h-[340px] overflow-y-auto py-2">
          {patients.length > 0 && (
            <Section title="Patients">
              {patients.map((p, i) => (
                ...
                >
                  <span className="font-mono text-[11px] text-[#5e6072] w-12 shrink-0">#{p.test_no}</span>
                  <span className="text-[13.5px] font-medium text-[#14151c] truncate">{p.title} {p.name}</span>
                  <span className="text-[12px] text-[#5e6072]">{p.age} {p.age_unit}</span>
          ...
          {pages.length > 0 && (
            <Section title="Go to">
              {pages.map(p => (
                ...
                >
                  <span className="w-7 h-7 rounded-lg bg-[#eef0f4] text-[#4c4e5d] flex items-center justify-center">
                    <p.icon size={14.5} />
                  </span>
                  {p.label}
                </button>
          ...
          {q && patients.length === 0 && pages.length === 0 && (
            <p className="px-5 py-10 text-center text-[13px] text-[#5e6072]">
              Nothing matches “{q}”
            </p>
          )}
        </div>
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="mb-1">
      <p className="px-5 pt-2 pb-1 text-[10.5px] font-semibold uppercase tracking-[0.09em] text-[#5e6072]">{title}</p>
      {children}
    </div>
  );
}
